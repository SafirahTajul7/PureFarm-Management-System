...
// Check if the soil test exists
if ($id > 0) {
    $soil_test = $soilTestManager->getSoilTest($id);
    
    if (!$soil_test) {
        // Redirect if soil test not found
    ...
    $moisture_percentage = $soil_test['moisture_percentage'];
    $temperature = $soil_test['temperature'];
    $nitrogen_level = $soil_test['nitrogen_level'];
    $phosphorus_level = $soil_test['phosphorus_level'];
    $potassium_level = $soil_test['potassium_level'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // If no errors, update the soil test in the database
    if (empty($errors)) {
        $result = $soilTestManager->updateSoilTest($id, [
            'field_id' => $field_id,
            'test_date' => $test_date,
            'ph_level' => $ph_level,
            'moisture_percentage' => $moisture_percentage,
        ...
        if ($result) {
            ...
            header('Location: soil_tests.php?updated=1');
            exit;
        } else {
...
$pageTitle = 'Edit Soil Test';
include 'includes/header.php';
?>

<div class="main-content">
    <div class="page-header">
        <h2><i class="fas fa-flask"></i> Edit Soil Test</h2>
    ...
    <div class="card">
        <div class="card-body">
            <form method="post" action="edit_soil_test.php?id=<?php echo $id; ?>">
                <div class="row mb-3">
                    <div class="col-md-6">
                ...
                <div class="text-end">
                    <a href="soil_tests.php" class="btn btn-secondary">Cancel</a>
                    <button type="submit" class="btn btn-primary">Update Soil Test</button>
                </div>
            </form>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Edit Soil Test page loaded');
